...
import { exportFoodsAction, importFoodsAction } from "@/actions/foods";
...
export function DataPortability() {
  ...
  async function handleExport() {
    setBusy(true);
    try {
      const payload = await exportFoodsAction();
      const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `hom-nay-an-gi-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      ...
      const result = await importFoodsAction(json);
      toast.success(`Đã nhập ${result.imported} món ăn.`);
    } catch {
